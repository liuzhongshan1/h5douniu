...
class Douniuplay extends Common
{
    ...
    public function index()
    {
        ...
        if ($room_id > 0) {
            ...
            $room = $room->toArray();
			$gamerule=unserialize($room['rule']);
			$gamenum = explode(':', $gamerule['gamenum']);
            $this->assign('gamenum', $gamenum[0]);
        ...
        foreach ($allmember as $v) {
            if ($v['gamestatus'] == 1 && $v['pai'] == '') {
                if($v['ratearr'] != ''){
                    ...
                    if(isset($ratearr[$playcountroom - 1])){
    ...
    public function theend()
    {
        ...
        $this->gamelock($this->memberinfo['room_id']);
        model('room')->gameinit(array('id' => $this->memberinfo['room_id']),$gamenum[0]);
        $this->gameunlock($this->memberinfo['room_id']);
